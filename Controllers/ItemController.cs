...
namespace MvcAppCollect.Controllers
{
...

    {
        ...
        public IActionResult Check(int id)
        {
            ...
            var categoryFromDb = _db.Items.FromSqlRaw("SELECT * FROM dbo.Items WHERE CollectionId = {0}", id).ToList();
            int list_count = categoryFromDb.Count;
            if (list_count == 0)
            {
                return RedirectToAction("Middleware", new { id = id });
            }
        ...
        public IActionResult Delete(int id, int CollectionId)
        {
            ...
            _db.SaveChanges();
            string s = CollectionId.ToString();
            Response.Redirect("https://rankinglist.azurewebsites.net/Item/Index/" + s);
